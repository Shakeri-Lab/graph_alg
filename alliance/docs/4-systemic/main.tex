...
measures describe what a firm \emph{has}, not what other firms would
\emph{lose} if the firm vanished.  We construct a
\emph{systemic-criticality meta-network} from the full-graph strategic
alliance panel, using canonical ultimate-parent CUSIP-6 identifiers and
partner-specific vulnerability scores.  The corrected 2017 snapshot
...
system-removal stress test.  The central conclusion is more cautious
than the prototype: the meta-network is a structural dependency map and
stress-testing device, not direct evidence that every top-ranked
partner exit would causally destroy value.
\end{abstract}
...
ingredients for a different question: \emph{Whose exit would matter to
many other firms at once?}
...
study coefficient.  The multiplier $S_{fpt}$ combines five
partner-specific features: tie strength, dyad tenure, redundancy
discount, partner layer-centrality percentile, and first-order
counterfactual betweenness exposure.
...
\caption{Top 30 systemic-critical firms in the corrected 2017
meta-network.  Bar length is aggregate predicted log-market-value loss
across focal firms that rank the partner in their top five.}
\label{fig:m1}
\end{figure}
...
partners).  Force-directed layout; partner node size $\propto$
in-degree; color encodes SIC2; gray nodes are focal firms.}
\label{fig:m4}
\end{figure}
...
produces an aggregate predicted dollar loss of 192{,}800 in Compustat
market-value units, 31.8 times the random-removal mean.  Removing the
...
% =====================================================================
\section{Downstream Operational Integration}
\label{sec:downstream}

The systemic-criticality panel is consumed by an operational
strategic-decision pipeline that runs per-firm reports for all 7,626
Compustat-matched focal firms.  The alignment recommender (Strategic
Question 1: ``which downstream partner should we form a
commercialization alliance with?'') uses two artifacts from this
paper as inputs.  First, the layer-specific exit weights of
Section~\ref{sec:method} weight the value side of the score.  Second,
the partner-level normalized in-degree from
Section~\ref{sec:findings} enters as a dependency-risk multiplier on
the candidate side: adding a partner that many other firms already
list as critical is penalized.  This propagates the systemic-criticality
view into individual firms' partner choices without re-deriving any of
the upstream weights.

\subsection{Persistence-aware durable-rent score}

The recommender's commercialization branch implements a
durable-rent score that combines structural opportunity, relational
capability, focal absorptive capacity, and dependency risk:
\begin{align}
...
in this paper's 2017 systemic-criticality cross-section, with a
``not in panel'' flag distinguishing structurally-zero risk from
unobserved risk.  $g(\text{R\&D}_f)$ is a per-focal absorptive-capacity
...
persistence-vs-acquisition asymmetry: the L$_2$ sales premium accrues
to sustained ties only, not to fresh acquisitions, so a candidate's
demonstrated tie-maintenance behavior is treated as a screening
device on top of the structural opportunity.
...
\emph{ranking} candidates uses $\text{brokerage}_{L_2}$ (the only
focal-specific signal in the current feature set), while the
\emph{value} attached to a chosen candidate uses the systemic panel
and the relational-capability features as multiplicative scalars.
The systemic-criticality panel from this paper retains its
operational role --- as a dependency-exposure overlay on the
brokerage-ranked top-$k$, not as the rank itself.
...
The recommender as currently designed is a tool for choosing among
already-established players; in a market where 72\% of realized new
$L_2$ ties involve a non-Compustat focal and another 12.6\% involve a
genuine new $L_2$ entrant, it cannot be a discovery engine.  The
0.2\% cross-layer-conversion share kills the alternative
``recommend conversions from prior non-$L_2$ ties'' hypothesis
empirically.
...
realized partner ($N = 119$ in-pool dyads, focal-clustered bootstrap
CI).  CIs in bold exclude the 0.5 random baseline.}
\label{tab:within-block}
\begin{tabular}{lrcc}
\toprule
Annotation & Mean $P^a$ & 95\% CI & Reading \\
\midrule
\texttt{n\_hist\_ties}                  & \textbf{0.820} & \textbf{[0.781, 0.857]} & realized partners are degree-heavy \\
\texttt{n\_current\_ties}               & \textbf{0.820} & \textbf{[0.772, 0.861]} & same \\
\texttt{candidate\_degree\_all}         & \textbf{0.819} & \textbf{[0.772, 0.861]} & same \\
\texttt{candidate\_degree\_l2}          & \textbf{0.745} & \textbf{[0.693, 0.793]} & moderate \\
\texttt{w\_tenure\_smooth}              & \textbf{0.221} & \textbf{[0.178, 0.274]} & realized partners are LESS tenured \\
\texttt{w\_redundancy}                  & \textbf{0.216} & \textbf{[0.174, 0.257]} & realized partners are HIGHER risk \\
\texttt{annotated\_value\_synthetic}    & \textbf{0.154} & \textbf{[0.119, 0.192]} & multiplicative durable-rent ANTI-correlates \\
\texttt{dep\_risk}                      & \textbf{0.084} & \textbf{[0.072, 0.096]} & realized partners are LESS systemic \\
\bottomrule
\end{tabular}
\end{table}

Three readings are forced by Table~\ref{tab:within-block}.  First,
candidate degree (current, historical, and $L_2$-specific) does
identify realized partners within the saturated brokerage frontier,
with mean percentile $\approx 0.82$ and tight CIs that exclude the
random baseline.  Second, the recommender's own durable-rent
annotations point in the wrong direction: realized partners are in
the bottom quintile of \texttt{w\_tenure\_smooth}, the bottom
quintile of \texttt{w\_redundancy}, and the bottom decile of
\texttt{dep\_risk}.  The multiplicative
\texttt{annotated\_value\_synthetic} compounds the two
anti-correlations to mean $P = 0.154$.  Third, while the Hankel-DMD
analysis says \emph{sustained} $L_2$ brokerage carries the sales
premium, this does not imply that already-tenured candidates should
be the partners selected for new ties; the empirical pattern is the
opposite.  Firms select fresh, low-history, non-systemic partners
when they form new $L_2$ alliances.  The recommender's value/risk
annotations should therefore be read as descriptive labels on the
...
dot-com era before relaxing.  The next empirical step is targeted
validation on the top systemic partners using exact deletion
recomputation and richer event-study designs around observed exits.
