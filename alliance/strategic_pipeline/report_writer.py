    ...
    with open(md_path, "w") as f:
        ...
        if goal == "innovation":
            ...
            f.write(
                ...
                "### How candidates are ranked (brokerage frontier with annotations)\n\n"
                "$\\text{rank}_{fc} \\;=\\; \\text{rank by } "
                "\\text{brokerage}_{L_2}(f, c) "
                "\\quad \\text{(focal-specific structural opportunity)}$\n\n"
                "Durability, absorptive capacity, and systemic-dependency "
                "risk are reported as a single annotation scalar:\n\n"
                "$\\text{annotated\\_value}(c) "
                "= \\text{brokerage}_{L_2}(f, c) \\;\\times\\; "
                "w_{\\text{tenure}}(c) \\;\\times\\; "
                "g(\\text{R\\&D}_f) \\;\\times\\; "
                "\\exp(-\\rho \\cdot \\text{DepRisk}(c))$\n\n"
                "**This annotation is NOT used to reorder the candidate "
                "list.** Earlier versions of the recommender multiplied "
                "these features into a `score_durable_rent` and ranked "
                "on it; the Week-2A diagnostic and the Week-2B "
                "out-of-time backtest both showed that this multiplicative "
                "score collapses to one universal candidate "
                "(Stanford for `raw_score` at 100% of focals; MIT under "
                "`w_tenure_only` at 99.96%).  The collapse destroys the "
                "focal-specific brokerage signal — the only feature in "
                "the current pipeline that depends on the focal firm.\n\n"
                "On the in-pool 2011 backtest, brokerage_only catches "
                "**15 of 16** realized partners at K=5; the multiplicative "
                "score catches **0 of 16**.  The recommender therefore "
                "ranks by brokerage and shows the durable-rent components "
                "as value/risk annotations on the frontier, not as inputs "
                "to the rank.\n\n"
                "Component definitions:\n\n"
                "- **brokerage_L2** ∈ [0, 1] (rank input) — Burt-style "
                "structural opportunity: fraction of candidate's L2 "
                "neighborhood *not* shared with the focal.  Saturates "
                "at 1.0 for ~99% of candidates in sparse-portfolio "
                "focals; within saturated tie-blocks the recommender's "
                "ordering is intentionally ambiguous (see "
                "`week2b_brokerage_tie_audit` for the quantification).\n"
                "- **w_tenure_smooth** ∈ (0, 1) (annotation) — "
                "Dyer-Singh-style relational capability: $\\sigma(z)$ "
                "where $z$ is the candidate's $\\log(1 + \\text{median tenure})$ "
                "z-scored against the SIC×L2 cohort baseline.\n"
                "- **g(R&D)** (per-focal annotation) — absorptive-capacity "
                "multiplier $1 + \\alpha \\cdot \\mathbf{1}\\{f \\in \\text{top-quartile R\\&D}\\}$, "
                "motivated by the L2 sales premium's R&D stratification "
                "(paper Section 4 / Figure 3B).\n"
                "- **DepRisk** ∈ [0, 1] (annotation) — candidate's "
                "normalized in-degree in the corrected "
                "systemic-criticality cross-section. High = many other "
                "firms already list this candidate as a top-5 critical "
                "partner.\n"
                "- **w_redundancy** = $\\exp(-1.5 \\cdot \\text{DepRisk})$ "
                "(annotation) — discount for adding a candidate already "
                "in the systemic critical-partner panel.\n"
                "- **annotated_value** = product of all four (annotation) "
                "— the durable-rent value of a selected candidate; "
                "*reported, not ranked*.\n\n"
            )
